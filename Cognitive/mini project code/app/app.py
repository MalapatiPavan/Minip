...
def save_submission(username):
    ...
    # Generate AI recommendations based on stress data
    avg_stress = health_summary.get("avg_stress", 0)
    # Fix None issue
    if avg_stress is None:
        avg_stress = 0
    st.session_state.ai_recommendations = get_recommendations(hsqsns, avg_stress, use_ai=True)
...
def render_exam_page(username):
    ...
    if st.session_state.test_submitted:
        ...
        if ai_recs:
            ...
            from recommendations import get_recommendations
            fallback_recs = get_recommendations(hsqsns, float(hs.get('avg_stress', 0)), use_ai=False)
            st.markdown("#### 📋 Recommendations")
            with st.container(border=True):
                st.markdown(fallback_recs)
